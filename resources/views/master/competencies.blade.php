...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold text-on-surface">Kompetensi</h1>
        </div>
        @if (auth()->user()->role === 'admin')
            <a href="{{ route('master.kompetensi.create') }}" class="rounded-lg accent-gradient px-3 py-2 text-sm font-medium text-white hover:opacity-90">Tambah</a>
        @endif
    </div>
            ...
            <tbody class="divide-y divide-outline-variant/20">
                @foreach ($items as $row)
                    <tr class="hover:bg-surface-container-low/80">
                        <td class="px-4 py-3 font-mono text-on-surface">{{ $row->kode_kompetensi }}</td>
                        <td class="px-4 py-3 text-on-surface-variant">{{ $row->group?->kode ?? '—' }}</td>
                        ...
                        @if (auth()->user()->role === 'admin')
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('master.kompetensi.edit', $row) }}" class="text-on-surface-variant underline">Ubah</a>
