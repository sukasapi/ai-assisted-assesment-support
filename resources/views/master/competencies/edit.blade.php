...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-zinc-900">Ubah kompetensi</h1>
    </div>
    <form method="POST" action="{{ route('master.kompetensi.update', $item) }}" class="max-w-lg space-y-4 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <input type="text" name="kode_kompetensi" id="kode_kompetensi" value="{{ old('kode_kompetensi', $item->kode_kompetensi) }}" required maxlength="32" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="definisi" id="definisi" rows="3" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">{{ old('definisi', $item->definisi) }}</textarea>
        </div>
        <div>
            <label for="tingkat_maksimum" class="block text-sm font-medium text-zinc-800">Tingkat maksimum</label>
            <input type="number" name="tingkat_maksimum" id="tingkat_maksimum" value="{{ old('tingkat_maksimum', $item->tingkat_maksimum) }}" min="1" max="20" required class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
