...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-zinc-900">Tambah tingkat kompetensi</h1>
    </div>
    <form method="POST" action="{{ route('master.tingkat-kompetensi.store') }}" class="max-w-lg space-y-4 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="number" name="tingkat" id="tingkat" value="{{ old('tingkat') }}" min="1" max="20" required class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label for="indikator_perilaku" class="block text-sm font-medium text-zinc-800">Indikator perilaku</label>
            <textarea name="indikator_perilaku" id="indikator_perilaku" rows="4" required class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">{{ old('indikator_perilaku') }}</textarea>
        </div>
        <div>
            <label for="etiket" class="block text-sm font-medium text-zinc-800">Etiket (opsional)</label>
            <input type="text" name="etiket" id="etiket" value="{{ old('etiket') }}" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label for="deskripsi" class="block text-sm font-medium text-zinc-800">Deskripsi (opsional)</label>
            <textarea name="deskripsi" id="deskripsi" rows="2" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">{{ old('deskripsi') }}</textarea>
        </div>
