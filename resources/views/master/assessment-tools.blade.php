...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        @if (auth()->user()->role === 'admin')
            <a href="{{ route('master.alat-penilaian.create') }}" class="rounded-lg accent-gradient px-3 py-2 text-sm font-medium text-white hover:opacity-90">Tambah</a>
        @endif
    </div>
                    ...
                    <tr class="hover:bg-surface-container-low/80">
                        ...
                        @if (auth()->user()->role === 'admin')
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('master.alat-penilaian.edit', $row) }}" class="text-on-surface-variant underline">Ubah</a>
