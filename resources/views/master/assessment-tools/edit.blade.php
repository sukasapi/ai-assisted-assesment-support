...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-zinc-900">Ubah alat penilaian</h1>
    </div>
    <form method="POST" action="{{ route('master.alat-penilaian.update', $item) }}" class="max-w-lg space-y-4 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
        <div>
            <label for="kode" class="block text-sm font-medium text-zinc-800">Kode</label>
            <input type="text" name="kode" id="kode" value="{{ old('kode', $item->kode) }}" required maxlength="32" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="3" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">{{ old('deskripsi', $item->deskripsi) }}</textarea>
        </div>
        <div>
            <label for="urutan" class="block text-sm font-medium text-zinc-800">Urutan</label>
            <input type="number" name="urutan" id="urutan" value="{{ old('urutan', $item->urutan) }}" min="0" required class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
