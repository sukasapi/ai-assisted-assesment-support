...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-zinc-900">Tambah peserta</h1>
    </div>
    <form method="POST" action="{{ route('master.peserta.store') }}" class="max-w-lg space-y-4 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        @csrf
        <div>
            <label for="kode_peserta" class="block text-sm font-medium text-zinc-800">Kode peserta</label>
            <input type="text" name="kode_peserta" id="kode_peserta" value="{{ old('kode_peserta') }}" required class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label for="nama_lengkap" class="block text-sm font-medium text-zinc-800">Nama lengkap</label>
            <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap') }}" required class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label for="alamat_surel" class="block text-sm font-medium text-zinc-800">Alamat surel</label>
            <input type="email" name="alamat_surel" id="alamat_surel" value="{{ old('alamat_surel') }}" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label for="jabatan" class="block text-sm font-medium text-zinc-800">Jabatan</label>
            <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan') }}" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label for="pendidikan" class="block text-sm font-medium text-zinc-800">Pendidikan</label>
            <input type="text" name="pendidikan" id="pendidikan" value="{{ old('pendidikan') }}" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label for="tanggal_lahir" class="block text-sm font-medium text-zinc-800">Tanggal lahir</label>
            <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir') }}" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="catatan" id="catatan" rows="2" class="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">{{ old('catatan') }}</textarea>
        </div>
