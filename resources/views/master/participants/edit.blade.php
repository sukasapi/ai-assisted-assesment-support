...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-on-surface">Ubah peserta</h1>
    </div>
    <form method="POST" action="{{ route('master.peserta.update', $item) }}" class="max-w-lg space-y-4 rounded-lg border border-outline-variant/40 bg-surface-container-lowest p-6 shadow-sm">
        @csrf
        @method('PUT')
        <div>
            <label for="kode_peserta" class="block text-sm font-medium text-on-surface">Kode peserta</label>
            <input type="text" name="kode_peserta" id="kode_peserta" value="{{ old('kode_peserta', $item->kode_peserta) }}" required class="mt-1 w-full rounded-md border border-outline-variant px-3 py-2 text-sm">
        </div>
        <div>
            <label for="nama_lengkap" class="block text-sm font-medium text-on-surface">Nama lengkap</label>
            <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap', $item->nama_lengkap) }}" required class="mt-1 w-full rounded-md border border-outline-variant px-3 py-2 text-sm">
        </div>
        <div>
            <label for="alamat_surel" class="block text-sm font-medium text-on-surface">Alamat surel</label>
            <input type="email" name="alamat_surel" id="alamat_surel" value="{{ old('alamat_surel', $item->alamat_surel) }}" class="mt-1 w-full rounded-md border border-outline-variant px-3 py-2 text-sm">
        </div>
        <div>
            <label for="jabatan" class="block text-sm font-medium text-on-surface">Jabatan</label>
            <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan', $item->jabatan) }}" class="mt-1 w-full rounded-md border border-outline-variant px-3 py-2 text-sm">
        </div>
        <div>
            <label for="pendidikan" class="block text-sm font-medium text-on-surface">Pendidikan</label>
            <input type="text" name="pendidikan" id="pendidikan" value="{{ old('pendidikan', $item->pendidikan) }}" class="mt-1 w-full rounded-md border border-outline-variant px-3 py-2 text-sm">
        </div>
        <div>
            <label for="tanggal_lahir" class="block text-sm font-medium text-on-surface">Tanggal lahir</label>
            <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir', $item->tanggal_lahir?->format('Y-m-d')) }}" class="mt-1 w-full rounded-md border border-outline-variant px-3 py-2 text-sm">
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="catatan" id="catatan" rows="2" class="mt-1 w-full rounded-md border border-outline-variant px-3 py-2 text-sm">{{ old('catatan', $item->catatan) }}</textarea>
        </div>
